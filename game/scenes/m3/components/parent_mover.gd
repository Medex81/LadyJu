...
var moving_to_rect:Rect2i = Rect2i(0, 0, 0, 0)
...
func _is_occupied(rect:Rect2i)->bool:
	for mover in get_tree().get_nodes_in_group(mover_group):
		if mover != self and mover.is_moved and rect.intersects(mover.moving_to_rect):
			return true
	return false

func check_move_all():
	for mover in get_tree().get_nodes_in_group(mover_group):
		if not mover.is_moved and mover.is_active: 
			mover.call_deferred("move")
...
func move(_direct:Vector2i = Vector2i.ZERO, is_automove:bool = true):
	...
	moving_to_rect = Rect2i(Vector2i(global_position) + _direct, _to_down_right)
	# двигаем родителя через твин по свойству позиции
	var move_tween = get_tree().create_tween()
	move_tween.tween_property(_parent, "global_position", global_position + Vector2(_direct) , _move_time)
